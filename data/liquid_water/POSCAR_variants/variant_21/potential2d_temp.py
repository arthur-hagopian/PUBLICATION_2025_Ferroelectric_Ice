...
P1_FRAC = (0.4129598875179568, 0.118791836901039, 0.3790767099002537)  # O
P2_FRAC = (0.3099848189996806, 0.0980637602013275, 0.3638040122670603)  # H1
P3_FRAC = (0.4293543489999934, 0.105033163863105, 0.2752040265310924)  # H2
...
def plot_plane(lat, pot):
    ...
    if COORD_MODE == "cartesian":
        ...
        xlabel, ylabel = "In-plane coordinate u (Å)", "In-plane coordinate v (Å)"
        coords = [
            (u_coords[0], v_coords[0]),
            (u_coords[1], v_coords[1]),
            (u_coords[2], v_coords[2]),
            (np.nan, np.nan)  # COM to be added
        ]
        com_vec = com_frac - p1
    ...
    cbar.set_label('Electrostatic potential (eV)', fontsize=14)
    ...
    for (x,y,label) in zip(coords, coords, labels):
        ...
        ax.text(x[0], x[1], " "+label, color='k', fontsize=13,
                verticalalignment='bottom', horizontalalignment='left')

    plt.savefig("potential2d_plane.png", dpi=300)
    plt.show()
