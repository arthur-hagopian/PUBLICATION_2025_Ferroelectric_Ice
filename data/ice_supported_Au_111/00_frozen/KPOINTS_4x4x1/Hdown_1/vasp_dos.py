...
def parse_doscar(filename):
    with open(filename, 'r') as f:
        ...
        total_dos_for_int = total_dos[mask]
        charge = np.trapz(energies_for_int,total_dos_for_int)
    ...
    return energies, total_dos, projected_dos, num_atoms, charge
...
def plot_dos(energies, total_dos, projected_dos, selected_atoms):
    ...
    for atom_idx in selected_atoms:
        ...
        total_dos += atom_dos
    plt.plot(energies, total_dos, label=f'Atom {atom_idx}', color='black')
    # Plot settings
    plt.xlim([-22,8])
...
if __name__ == '__main__':
    ...
    energies, total_dos, projected_dos, num_atoms, charge = parse_doscar(doscar_file)
    ...
    selected_atoms = list(range(101, 173))
